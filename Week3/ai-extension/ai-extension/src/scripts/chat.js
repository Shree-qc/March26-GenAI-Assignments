...
class ChatUI {
    ...
    getPromptKeys(language, engine) {
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                // For non-Java/Selenium or non-TypeScript/Playwright combinations, generate separately
    ...
    isPythonSelenium(language, engine) {
        return (language === 'python' || language === 'py') && engine === 'selenium';
    }
    ...
    // playwright supports typescript natively
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Only Java/Selenium and TypeScript/Playwright are currently available.`;
