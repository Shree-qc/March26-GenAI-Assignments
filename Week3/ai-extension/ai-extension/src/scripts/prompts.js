...
\      | "admin"    | "admin123"|
  ...
  PLAYWRIGHT_TYPESCRIPT_PAGE_ONLY: `
    ...
    - Use Playwright 1.40+ compatible imports and syntax.
    - Use meaningful method names with async/await.
    ...
    Persona:
    - Audience: Automation engineer focusing on maintainable POM structure with modern TypeScript.

    Output Format:
    - A single TypeScript class inside a \`\`\`typescript\`\`\` block.

    Tone:
    - Clean, maintainable, modern TypeScript with Playwright best practices.
  `,

  /**
   * Playwright TypeScript with Cucumber Steps
   */
  PLAYWRIGHT_TYPESCRIPT_WITH_CUCUMBER: `
    ...
    - Do NOT include Page Object code.
    - Step defs must include Playwright setup, proper async/await, and actual Playwright code.
    - Use Scenario Outline with Examples table.
    - Use South India realistic dataset (names, addresses, pin codes, mobile numbers).
    - Use dropdown values only from provided DOM.
    - Generate multiple scenarios if applicable.
    ...
    Tone:
    - Professional, executable, modern async/await TypeScript with Playwright.
  `
};
