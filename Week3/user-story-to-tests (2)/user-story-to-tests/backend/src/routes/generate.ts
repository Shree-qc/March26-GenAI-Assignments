...
generateRouter.post('/', async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    // Fetch user story details from Jira
    const jiraClient = new JiraClient()
    const userStory = await jiraClient.getUserStory(request.issueKey)

    if (!userStory) {
      res.status(404).json({
        error: 'User story not found in Jira'
      })
      return
    }

    // Build prompts
    const userPrompt = buildPrompt(userStory, request.additionalInfo)
